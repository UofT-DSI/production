...
- A sequence of operations reading from one or more storage types, combined with data transformation procedures, creates a *pipeline*.
...
- Example: parquet.
- Accessing columns will be faster than accessing rows.
...
- Normalization involves determining how much redundancy exists in a table and reducing it, as required.
...
- We use a query language to specify the data that we want from a database.
...
![bg contain right:40%](./images/02_matrix_network.png)
...
- Graph models enable network or graph metrics: 
    ...
    - Node metrics like centrality measures: degree, eigen, and betweenness.
...
|Schema changes will cause significant trouble. |No need to worry about schema changes (yet), as the worry is shifted to the downstream applications that use this data|
...
## Data Storage and Processing (cont.)
...
- Transactional databases usually offer ACID guarantees:
    ...
    - Isolation: Two transactions happen at the same time as if they were isolated. Two users accessing the same data will not change it simultaneously.
...
## Transactional vs Analytical DB (cont.)


OLTP and OLAP are terms falling out of use, since the divide is somewhat outdated:

1. The separation was due to technological limitations: transactional databases that can handle analytical queries efficiently (e.g., CockroachDB)
...
ETL is the process of extracting data from one or several sources, transforming it into the shape that an application or model requires, and loading it into a desired destination.
...
- It is inefficient to search through a massive amount of raw data for your desired data.
...
- Process 1 writes to the DB, Process 2 reads from the same DB.
...
- REST is commonly used for public APIs and is implemented over HTTP.
...
## Events and Types of Real-Time Transports (cont.)
...
## The Publish-Subscribe (pubsub) Model
...
- Computation can also be started periodically, but the periods are generally shorter. Computation could also be started when needed.
- Stream processing is performed on rapidly changing variables known as dynamic features (for example, average metric in the past 5 minutes).
